...
INP=$UPATH/src/StandAlone/inputs/PhaseField/heat/be_struct_conv

for UPS in $INP/be_struct_conv_%(solver)s_3d_nlvl*_%(ph)d_*.ups; do
    LOG=$(basename $UPS).log
    [[ -f $LOG && -s $LOG ]] || mpirun -np %(np)d $SUS $UPS &> $LOG
done
"""

NAME = "be_struct_conv_%(solver)s_3d_np%(np)d"
