...
inline HYPRE_Int
HYPREDBG_SStructMatrixAddToValues ( HYPRE_SStructMatrix   matrix,
                                    HYPRE_Int             part,
                                    HYPRE_Int           * index,
                                    HYPRE_Int             var,
                                    HYPRE_Int             nentries,
...
{
#if HYPREDBG_NDIM == 1
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_SStructMatrixAddToValues ( " << matrix << ", " << part << ", [" << index[0] << "], " << var << ", " << nentries << ", [";
#elif HYPREDBG_NDIM == 2
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_SStructMatrixAddToValues ( " << matrix << ", " << part << ", [" << index[0] << ","  << index[1] << "], " << var << ", " << nentries << ", [";
#elif HYPREDBG_NDIM == 3
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_SStructMatrixAddToValues ( " << matrix << ", " << part << ", [" << index[0] << ","  << index[1] << ","  << index[2] << "], " << var << ", " << nentries << ", [";
#endif
    for ( int e = 0; e < nentries - 1; ++e )
        ss << entries[e] << ",";
    ss << entries[nentries - 1] << "], [";
    for ( int s = 0; s < nentries - 1; ++s )
        ss << values[s] << ",";
    ss << values[nentries - 1] << "] )" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_SStructMatrixAddToValues ( matrix, part, index, var, nentries, entries, values );
}
...
#if 0
inline HYPRE_Int
HYPREDBG_SStructGMRESCreate ( MPI_Comm              comm,
                              HYPRE_SStructSolver * solver )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_SStructGMRESCreate ( comm, solver )" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_SStructGMRESCreate ( comm, solver );
}
#endif

inline HYPRE_Int
HYPREDBG_GMRESSetup ( HYPRE_Solver solver,
                      HYPRE_Matrix A,
                      HYPRE_Vector b,
                      HYPRE_Vector x )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetup ( " << solver << ", " << A << ", " << b << ", " << x << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetup ( solver, A, b, x );
}

inline HYPRE_Int
HYPREDBG_GMRESSolve ( HYPRE_Solver solver,
                      HYPRE_Matrix A,
                      HYPRE_Vector b,
                      HYPRE_Vector x )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSolve ( " << solver << ", " << A << ", " << b << ", " << x << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSolve ( solver, A, b, x );
}


inline HYPRE_Int
HYPREDBG_GMRESSetKDim ( HYPRE_Solver solver,
                        HYPRE_Int    k_dim )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetKDim ( " << solver << ", " << k_dim << " )" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetKDim ( solver, k_dim );
}

inline HYPRE_Int
HYPREDBG_GMRESSetTol( HYPRE_Solver solver,
                      HYPRE_Real   tol )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetTol ( " << solver << ", " << tol << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetTol ( solver, tol );
}

inline HYPRE_Int
HYPREDBG_GMRESSetAbsoluteTol ( HYPRE_Solver solver,
                               HYPRE_Real   a_tol )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetAbsoluteTol ( " << solver << ", " << a_tol << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetAbsoluteTol ( solver, a_tol );
}

inline HYPRE_Int
HYPREDBG_GMRESSetConvergenceFactorTol ( HYPRE_Solver solver,
                                        HYPRE_Real   cf_tol )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetConvergenceFactorTol ( " << solver << ", " << cf_tol << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetConvergenceFactorTol ( solver, cf_tol );
}

inline HYPRE_Int
HYPREDBG_GMRESSetMinIter( HYPRE_Solver solver,
                          HYPRE_Int    min_iter )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetMinIter ( " << solver << ", " << min_iter << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetMinIter ( solver, min_iter );
}

inline HYPRE_Int
HYPREDBG_GMRESSetMaxIter( HYPRE_Solver solver,
                          HYPRE_Int    max_iter )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetMaxIter ( " << solver << ", " << max_iter << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetMaxIter ( solver, max_iter );
}

inline HYPRE_Int
HYPREDBG_GMRESSetStopCrit( HYPRE_Solver solver,
                           HYPRE_Int    stop_crit )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetStopCrit ( " << solver << ", " << stop_crit << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetStopCrit ( solver, stop_crit );
}

inline HYPRE_Int
HYPREDBG_GMRESSetRelChange( HYPRE_Solver solver,
                            HYPRE_Int    rel_change )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetRelChange ( " << solver << ", " << rel_change << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetRelChange ( solver, rel_change );
}

inline HYPRE_Int
HYPREDBG_GMRESSetSkipRealResidualCheck( HYPRE_Solver solver,
                                        HYPRE_Int    skip_real_r_check )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetSkipRealResidualCheck ( " << solver << ", " << skip_real_r_check << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetSkipRealResidualCheck ( solver, skip_real_r_check );
}

inline HYPRE_Int
HYPREDBG_GMRESSetPrecond( HYPRE_Solver         solver,
                          HYPRE_PtrToSolverFcn precond,
                          HYPRE_PtrToSolverFcn precond_setup,
                          HYPRE_Solver         precond_solver )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetPrecond ( " << solver << ", precond, precond_setup, " << precond_solver << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetPrecond ( solver, precond, precond_setup, precond_solver );
}

inline HYPRE_Int
HYPREDBG_GMRESSetLogging( HYPRE_Solver solver,
                          HYPRE_Int    level )
{
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESSetLogging ( " << solver << ", " << level << " );" << std::endl; printf("%s",ss.str().c_str());
    return HYPRE_GMRESSetLogging ( solver, level );
}

inline HYPRE_Int
HYPREDBG_GMRESGetNumIterations ( HYPRE_Solver   solver,
                                 HYPRE_Int    * num_iterations )
{
    HYPRE_Int res = HYPRE_GMRESGetNumIterations ( solver, num_iterations );
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESGetNumIterations ( " << solver << ", " << *num_iterations << " )" << std::endl; printf("%s",ss.str().c_str());
    return res;
}

inline HYPRE_Int
HYPREDBG_GMRESGetFinalRelativeResidualNorm ( HYPRE_Solver   solver,
                                             HYPRE_Real   * norm )
{
    HYPRE_Int res = HYPRE_GMRESGetFinalRelativeResidualNorm ( solver, norm );
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESGetFinalRelativeResidualNorm ( " << solver << ", " << *norm << " )" << std::endl; printf("%s",ss.str().c_str());
    return res;
}

inline HYPRE_Int
HYPREDBG_GMRESGetResidual( HYPRE_Solver   solver,
                        void         * residual )
{
    HYPRE_Int res = HYPRE_GMRESGetResidual ( solver, residual );
    int world_rank; MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); std::stringstream ss; ss << world_rank << "HYPRE_GMRESGetResidual ( " << solver << ", " << residual << " )" << std::endl; printf("%s",ss.str().c_str());
   return res;
}
